...
import { Coin } from "@atomone/atomone-types/cosmos/base/v1beta1/coin.js";
import { ParameterChangeProposal } from "@atomone/atomone-types/cosmos/params/v1beta1/params.js";
import {
  QueryPoolRequest,
  QueryPoolResponse,
} from "@atomone/atomone-types/cosmos/staking/v1beta1/query.js";
import { Pool } from "@atomone/atomone-types/cosmos/staking/v1beta1/staking.js";
import { SoftwareUpgradeProposal } from "@atomone/atomone-types/cosmos/upgrade/v1beta1/upgrade.js";
import { Any } from "@atomone/atomone-types/google/protobuf/any.js";
...
export class GovModule implements Types.IndexingModule {
  ...
  this.indexer.on("/atomone.gov.v1.MsgSubmitProposal", async (event) => {
      ...
      const content = prop.messages.length>0 ? JSONStringify(
         prop.messages.map((x) => {
  ...
  async deleteProposal(proposalId: bigint) {
    ...
    await db.query("DELETE FROM proposal_tally_results WHERE proposal_id=$1", [
      proposalId.toString(),
    ]);
    await db.query("DELETE FROM proposal_deposits WHERE proposal_id=$1", [
      proposalId.toString(),
    ]);
